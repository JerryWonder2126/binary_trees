...
size_t s_tree_height(const binary_tree_t *tree)
{
	...
	left = tree->left ? 1 + s_tree_height(tree->left) : 1;
	right = tree->right ? 1 + s_tree_height(tree->right) : 1;
...
int binary_tree_is_perfect(const binary_tree_t *tree)
{
	...
	if (tree == NULL)
		return (0);
	leftHeight = s_tree_height(tree->left);
	rightHeight = s_tree_height(tree->right);
